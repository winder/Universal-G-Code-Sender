...
@ActionID(
        ...
        id = "com.willwinder.ugs.nbp.jog.actions.SetMachineDecimalFormatFive")
@ActionRegistration(
        displayName = "resources.MessagesBundle#platform.plugin.jog.setMachineDecimalFormatFive",
        lazy = false)
@ActionReferences({
        @ActionReference(
                path = LocalizingService.MENU_MACHINE_JOG_DECIMAL_PLACES_5,
                position = 10000,
                separatorAfter = 10001)
})
@ServiceProvider(service = SetMachineDecimalFormatFive.class)
public class SetMachineDecimalFormatFive extends AbstractAction implements Presenter.Menu {
    private final String DECIMAL_FORMAT_FIVE = "0.00000";
    private final BackendAPI backend;
    private final JCheckBoxMenuItem menuItem;

    public SetMachineDecimalFormatFive() {
        String title = Localization.getString("platform.plugin.jog.setMachineDecimalFormatFive");
        putValue(NAME, title);
